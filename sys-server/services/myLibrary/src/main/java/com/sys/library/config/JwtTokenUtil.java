...
/**
 * @author ：Li.Yong
 * @date ：Created in 2022/9/17 14:21
 * @description：
 */
@Component
@Slf4j
public class JwtTokenUtil {
    ...
    private Claims getClaimsFromToken(String token) {
        ...
        try {
            ...
            log.info("JWT格式验证失败:{}-{}",token,e.getMessage());
    ...
    /**
     * 根据用户信息生成token
     * JWT构成:  Header（头部）Payload（负载）Signature（签名）
     *
     */
    public String generateToken(UserAuthDetails userDetails) {
        Map<String, Object> claims = new HashMap<>();
        claims.put(CLAIM_KEY_USER_ID, userDetails.getUser().getUserName());
